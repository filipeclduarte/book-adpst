...
```python
df_raw = pd.read_csv('../notebooks/mortality_sweden.csv')
...
```python
train_size = int(len(df) * 0.8)   # 1950–2005
train = df.iloc[:train_size]
test  = df.iloc[train_size:]       # 2006–2020
```
...
naive_pred = np.full(len(test), train['taxa_mortalidade'].iloc[-1])

# SES — suavização exponencial simples
ses_pred = SimpleExpSmoothing(train['taxa_mortalidade'])\
    .fit(optimized=True).forecast(len(test))

# Holt — tendência aditiva amortecida (correto para série anual sem sazonalidade)
holt_pred = ExponentialSmoothing(
    train['taxa_mortalidade'], trend='add', damped_trend=True
).fit(optimized=True).forecast(len(test))

# ARIMA(1,1,1) — passeio aleatório com estrutura AR e MA
arima_pred = ARIMA(train['taxa_mortalidade'], order=(1,1,1))\
...
for nome, fc in previsoes.items():
    m = mase(test['taxa_mortalidade'].values, np.asarray(fc),
             train['taxa_mortalidade'].values)
...
ax1.plot(df.index, df['taxa_mortalidade'], 'k-', label='Observado', linewidth=1.5)
cores = {'Naive': '#e41a1c', 'SES': '#377eb8', 'Holt': '#4daf4a', 'ARIMA': '#984ea3'}
for nome, fc in previsoes.items():
    ax1.plot(anos_teste, np.asarray(fc), '--', color=cores[nome],
             label=nome, linewidth=1.2)
ax1.axvline(anos_teste[0], color='gray', linestyle=':', linewidth=1)
ax1.set_ylabel('Taxa de mortalidade (‰)')
ax1.legend(ncol=2, fontsize=9)

# Painel inferior: erros de previsão
for nome, fc in previsoes.items():
    erros = test['taxa_mortalidade'].values - np.asarray(fc)
...
plt.tight_layout()
plt.savefig('../images/fig-baselines-mortalidade.png', dpi=150, bbox_inches='tight')
plt.show()
```
...
step      = 3    # avança 3 anos entre dobras

n = len(df)
resultados = []

for start in range(min_train, n - horizon, step):
    tr = df.iloc[:start]['taxa_mortalidade']
    te = df.iloc[start:start + horizon]['taxa_mortalidade']
...
O parâmetro `step=3` gera uma dobra a cada 3 anos — suficiente para capturar a variabilidade de desempenho em séries demográficas sem custo computacional excessivo.
...
residuos = test['taxa_mortalidade'].values - np.asarray(holt_pred)
